...
3. **Update `docs/PROGRESS.md`** with one line for this phase: phase number,
   ...
   phase rows.
4. **Write the phase file's `## Handoff for next session`** paragraph: where
   things stand at close, and what the next phase should do first. Replace
   any placeholder text that is there.
5. **Set the phase file's `Status` header to `done`.**
6. **Advance `docs/plans/current-phase.md`** to point at the next phase file
...
7. **Remind the user to commit** as `phase-NN: phase complete` on the
