...
if [ -x "$repo_root/scripts/checks/check-comment-style.sh" ]; then
  findings="$("$repo_root/scripts/checks/check-comment-style.sh" --files "$file_path" 2>&1)" || {
    printf '%s\n' "$findings" >&2
    exit 2
  }
fi
...
if [ -x "$repo_root/scripts/checks/check-default-style.sh" ]; then
  findings="$("$repo_root/scripts/checks/check-default-style.sh" "$file_path" 2>&1)" || {
    printf '%s\n' "$findings" >&2
    exit 2
  }
fi
...
if [ -x "$repo_root/scripts/checks/check-typed-status.sh" ]; then
  findings="$("$repo_root/scripts/checks/check-typed-status.sh" "$file_path" 2>&1)" || {
    printf '%s\n' "$findings" >&2
    exit 2
  }
fi
