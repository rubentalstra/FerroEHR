---
name: viewer-wire-consumption
description: Confirmed defect patterns when auditing app/ferroehr-viewer as an ITS-REST client (AQL escaping, datetime-local UTC, 204 branches, strict DTOs, misattributed OAS-vs-docs-text citations)
metadata:
  type: project
---
...
- **AQL string escaping is BACKSLASH, never SQL doubling.** `AqlLexer.g4`
  ...
  is the correct in-repo escaper (`\'`). The viewer's `pages/system.rs`
...
- **Two CDR error-body shapes**: `{message, validationErrors[]}` (the released
  ...
  shape (`{message, code, errors[DV_CODED_TEXT]}`) — unadjudicated. The viewer's
...
- **Citation trap: OAS artifacts vs docs text.** "The format is always an
  ...
  representation. Viewer files misattribute all four.
- **`composition_get` declares `204` (`204_deleted_at_time`); `ehr_status_get_at_time`
  ...
  (`api/ehr/composition.rs` `if body.is_null()`); the viewer's
...
- **`CompositionDeleteParams` has no `if_match` field** — the CDR never
  ...
  `person_delete`). The viewer sends one anyway, equal to the path segment,
  so it is inert.
...
- `Options.yaml` (System API manifest) marks EVERY property optional; the
  viewer deserializes into a struct with no container `#[serde(default)]` →
...
- The typed-status guard (`scripts/checks/typed-status.sh`) greps only
  ...
  `ViewerError::Cdr { status: 409, .. }` patterns, of which the viewer has ~40.
