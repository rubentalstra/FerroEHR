...
The service error → wire status chain in `app/ferroehr`:

- `app/ferroehr/src/service/error.rs` — `impl From<ServiceError> for SmError` carries the authoritative table (NotFound→VersionedObjectDoesNotExist→404, VersionConflict→VersionMismatch→412, Conflict→CompositionAlreadyExists→409, Unprocessable/ValidationFailed→ContentInvalid→422, BadRequest→PreconditionViolation→400). `ServiceError::sm(status, msg)` is the inbound (SM→ServiceError) mirror.
- `app/ferroehr/src/storage/error.rs::classify_sqlx` — raw `sqlx::Error` classification: **SQLSTATE class 23 (integrity/FK/unique) → Conflict → 409**; 40001/40P01 (serialization/deadlock) → 409; PoolTimedOut → ServiceOverloaded → 503; everything else → Exception → 500.
