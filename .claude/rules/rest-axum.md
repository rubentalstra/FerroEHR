...
- **Implement the generated server traits.** One `impl` per API group
  ...
  bodies are the ported EHRbase behaviour (the service layer, P12). RM payloads
...
- **Errors → responses** via `openehr-its::rest::runtime::ApiError` (it carries
  ...
  openEHR error body shape EHRbase returns.
- **OpenAPI:** `utoipa` may emit our OAS as a CI **drift-check** against the
  vendored upstream OAS — a parity signal, never the source of truth (the
...
Behaviour is verified by the parity harness (P19), not by mirroring EHRbase's
