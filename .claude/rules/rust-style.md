...
Enforcement (tier 4): `scripts/check-default-style.sh` — per-edit via the
...
|---|---|---|
| 0505 + 1574 | comment/doc conventions | `comments.md` + `scripts/check-comment-style.sh` + `too_long_first_doc_paragraph` |
| 3681 | a field's default lives inline in its `Default` impl | §Default values above + `scripts/check-default-style.sh` |
