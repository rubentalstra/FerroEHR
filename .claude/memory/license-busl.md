...
**How to apply:** new first-party files carry `SPDX-FileCopyrightText: Ruben Talstra` + `SPDX-License-Identifier: BUSL-1.1`; files under `crates/openehr-*` and their codegen templates carry `Apache-2.0` instead (owner decision 2026-09-03: the spec crates stay open source so anyone can use them); never call the project open source, say source-available; `scripts/checks/licensing-declarations.sh` fails on a stale MIT claim. The grant wording is the owner's legal text: propose changes, never edit it on your own. FerroTERM's grant wording differs (it permits "products and services you operate"); do not copy it here. See [[owner-work-style]].
