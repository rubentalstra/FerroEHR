...
- A boot warning when authentication is enabled but no mechanism is configured: the server now says so prominently at startup (and names the ways out — add `[[auth.basic.users]]`, add an `[auth.oidc]` issuer, or set `auth.enabled = false`) instead of silently refusing every request with `401`.
