...
- **Signed build provenance and SBOM attestations** on every published artifact. The images already carried BuildKit's SLSA provenance and an SPDX SBOM but unsigned — readable, not verifiable; they are now signed through Sigstore, and the release binaries gained both from nothing. Verify with `gh attestation verify oci://ghcr.io/rubentalstra/ferroehr:TAG -R rubentalstra/FerroEHR`. Each release also attaches a CycloneDX SBOM of the Rust dependency graph with `pkg:cargo/…` purls, licences, checksums and the direct-versus-transitive dependency edges. The isolation-based SLSA build levels are deliberately not claimed, since the provenance is generated in the job that builds.
