...
```bash
export EHRBASE_REST_SMART__ENABLED=true
export EHRBASE_REST_SMART__ENDPOINTS__AUTHORIZATION_ENDPOINT=https://as.example/auth
export EHRBASE_REST_SMART__ENDPOINTS__TOKEN_ENDPOINT=https://as.example/token
```
...
The full key set (prefix `EHRBASE_REST_SMART__`, `__` separates nested
fields; part of the REST config, so the `EHRBASE_REST_CONFIG` TOML file can
carry it as a `[smart]` block):

| Key | Default | Meaning |
|---|---|---|
| `EHRBASE_REST_SMART__ENABLED` | `false` | Master switch. Off = no discovery document (404) and an inert scope gate. |
| `EHRBASE_REST_SMART__PLATFORM_BASE_URL` | unset | Base the discovery document hangs off. Unset = the REST root (`/ehrbase/rest`). A leading path is honoured (`/gateway/v1` → `/gateway/v1/.well-known/smart-configuration`). |
| `EHRBASE_REST_SMART__EHR_ID_CLAIM` | `ehrId` | Token claim carrying the launch context's openEHR EHR id. |
| `EHRBASE_REST_SMART__PATIENT_CLAIM` | `patient` | Fallback launch-context claim when the EHR-id claim is absent. |
| `EHRBASE_REST_SMART__REQUIRE_SMART_SCOPES` | `false` | Fail-closed switch — see [Advisory vs required](#advisory-vs-required) below. |
| `EHRBASE_REST_SMART__EPISODE__ENABLED` | `false` | Advertise + accept episode launch context (experimental; advisory only, no episode filtering). |
| `EHRBASE_REST_SMART__LAUNCH_BASE64_JSON` | `false` | Advertise the base64-JSON launch-parameter capability (experimental; consumed by the app, not the CDR). |
| `EHRBASE_REST_SMART__ENDPOINTS__ISSUER` | unset | Advertised token issuer. Unset = falls back to the configured OIDC bearer issuer. |
| `EHRBASE_REST_SMART__ENDPOINTS__JWKS_URI` | unset | Advertised `jwks_uri`. |
| `EHRBASE_REST_SMART__ENDPOINTS__AUTHORIZATION_ENDPOINT` | unset | Advertised OAuth2 authorization endpoint. |
| `EHRBASE_REST_SMART__ENDPOINTS__TOKEN_ENDPOINT` | unset | Advertised OAuth2 token endpoint. |
| `EHRBASE_REST_SMART__ENDPOINTS__REGISTRATION_ENDPOINT` | unset | Advertised dynamic-client registration endpoint. |
| `EHRBASE_REST_SMART__ENDPOINTS__INTROSPECTION_ENDPOINT` | unset | Advertised token introspection endpoint. |
| `EHRBASE_REST_SMART__ENDPOINTS__REVOCATION_ENDPOINT` | unset | Advertised token revocation endpoint. |
| `EHRBASE_REST_SMART__ENDPOINTS__MANAGEMENT_ENDPOINT` | unset | Advertised user management endpoint. |
| `EHRBASE_REST_SMART__ENDPOINTS__TOKEN_ENDPOINT_AUTH_METHODS_SUPPORTED` | `[]` | Advertised client auth methods (e.g. `client_secret_basic`, `private_key_jwt`). |
| `EHRBASE_REST_SMART__ENDPOINTS__GRANT_TYPES_SUPPORTED` | `[]` | Advertised grant types. `implicit` and the password grant are deprecated in SMART and **rejected at boot**. |
| `EHRBASE_REST_SMART__ENDPOINTS__RESPONSE_TYPES_SUPPORTED` | `[]` | Advertised response types (e.g. `code`). |
| `EHRBASE_REST_SMART__ENDPOINTS__CODE_CHALLENGE_METHODS_SUPPORTED` | `[]` | Advertised PKCE methods (e.g. `S256`). |
| `EHRBASE_REST_SMART__ENDPOINTS__SCOPES_SUPPORTED` | `[]` | Advertised scopes. Empty = a default list reflecting what the CDR enforces. |
