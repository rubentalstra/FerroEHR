...
| [Art. 18 and 21](https://eur-lex.europa.eu/eli/reg/2016/679/oj) restriction and objection | A [restriction register](retention.md#restriction-of-processing) at whole-EHR or single-object grain, with the marked object refused on every read, query, export, event and write path while its stored rows stay untouched, and its lift stamped rather than erased ([#3324](https://github.com/rubentalstra/FerroEHR/issues/3324)); a [research objection](retention.md#objection-to-research-processing) under Art. 21(6) that removes the record from population queries, exports and the event stream while leaving reads for care untouched ([#3325](https://github.com/rubentalstra/FerroEHR/issues/3325)) | Decide when to set them, record the ground, hold the restriction in the systems around the CDR, and tell the subject before lifting it |
