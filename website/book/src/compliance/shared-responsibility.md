...
| [Art. 4(5)](https://eur-lex.europa.eu/eli/reg/2016/679/oj) pseudonymisation | Clinical and demographic data live in separate schemas with non-overlapping `NOINHERIT` roles, and the server refuses to boot if a role reaches across. The resolve map that rejoins them is not yet a domain of its own ([#3158](https://github.com/rubentalstra/FerroEHR/issues/3158)) | Give the demographic pool its own DSN, and keep the additional information outside the CDR until the linkage service lands |
