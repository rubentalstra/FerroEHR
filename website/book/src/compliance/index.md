...
| **Art. 17(1)** erasure, subject to the grounds in **17(3)(b) to (d)** | `DELETE {base}/admin/ehr/{ehr_id}` removes the EHR and everything it owns, compositions, `EHR_STATUS`, item tags, contributions and every historical version, over the primary and the cold archival tier alike ([physical deletion](../operations-admin-apis.md#physical-deletion)) | shipped | Decide whether a 17(3) ground refuses the request, a legal obligation, public health, or research under Art. 89(1), and record that decision. Backups and copies outside the CDR stay yours to reach |
| **Art. 18** restriction of processing, which Art. 4(3) defines as marking stored data to limit its future processing | `EHR_STATUS.is_queryable = false` takes the EHR out of AQL results and `is_modifiable = false` refuses content writes, so the mark exists at the whole record | shipped; a mark finer than the EHR, restricting one composition while the rest of the record stays in use, is planned, [#3324](https://github.com/rubentalstra/FerroEHR/issues/3324) | Record the 18(1) ground, hold the restriction in the systems around the CDR, and tell the subject before you lift it (18(3)) |
...
| **Art. 21(1)** objection, and **21(6)** objection to research processing under Art. 89(1) | An objection can be honoured at the whole record by setting `is_queryable = false`, which takes the EHR out of AQL and so out of cohort queries | shipped; a per-subject research objection the cohort queries and the secondary-use feed honour without withdrawing the record from care is planned, [#3325](https://github.com/rubentalstra/FerroEHR/issues/3325) | Weigh the compelling legitimate grounds 21(1) asks for, record the outcome, and carry the objection into the systems outside the CDR |
...
| **BDSG § 35 Abs. 2 and 3**, restriction of processing in place of erasure where erasure would harm the subject or a retention period bars it | An EHR whose status carries `is_queryable = false` leaves every AQL result and one carrying `is_modifiable = false` refuses content writes; a version can be marked deleted without destroying its history | shipped | Decide per request whether Art. 17 or the restriction applies, and notify the subject of a restriction; a record-level restriction finer than the EHR is the organisation's process on top of these flags |
...
| **GDNG § 6**, a provider's own patient data reprocessed for quality assurance, research and statistics: pseudonymised, anonymised as soon as possible, under a rights-and-roles concept with logging, deleted after thirty years at the latest, and disclosable to the subject by kind, scope and concrete purpose | Per-domain roles that keep a research credential off the identifying data, [cohort queries](../querying-aql.md#cohort-queries-across-the-pseudonymisation-boundary) named by a digest of their predicates and withheld under a small-cell threshold, and an access record per query carrying the deployment's declared `purpose` and `legal_basis` | shipped, [#3159](https://github.com/rubentalstra/FerroEHR/issues/3159); the rebuildable read model under project-level pseudonyms is planned, [#3160](https://github.com/rubentalstra/FerroEHR/issues/3160) | Write the rights-and-roles concept, publish the purposes, run the thirty-year clock, and answer subjects' requests from the trail |
