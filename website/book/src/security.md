...
What is deliberately **not** claimed: the isolation-based SLSA build levels. The
provenance is generated by the same job that builds, which is honest and
verifiable but is not the same thing as provenance produced somewhere the build
cannot reach. Claiming a level we do not meet would be worse than naming the
boundary.
