...
```shell
helm show chart oci://ghcr.io/rubentalstra/charts/ferroehr --version 8.2.2
```
...
|---|---|---|---|
| Chart version | templates, values schema, defaults | `--version 8.2.2` | SemVer over the chart's own contract |
| Image tag | the server binary | `--set image.tag=4.2.0` (or `image.digest`) | the application's SemVer line |
...
>
> ```shell
> helm template ferroehr oci://ghcr.io/rubentalstra/charts/ferroehr --version 8.2.2 \
...
helm upgrade ferroehr oci://ghcr.io/rubentalstra/charts/ferroehr \
  --version 8.2.2 -n ferroehr --reuse-values \
...
```shell
FERROEHR_IMAGE=ghcr.io/rubentalstra/ferroehr:4.2.0 \
