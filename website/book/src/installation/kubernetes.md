...
```shell
helm show chart oci://ghcr.io/rubentalstra/charts/ferroehr --version 6.0.30
```
...
|---|---|---|---|
| Chart version | templates, values schema, defaults | `--version 6.0.30` | SemVer over the chart's own contract |
| Image tag | the server binary | `--set image.tag=4.0.12` (or `image.digest`) | the application's SemVer line |
...
>
> ```shell
> helm template ferroehr oci://ghcr.io/rubentalstra/charts/ferroehr --version 6.0.30 \
...
helm upgrade ferroehr oci://ghcr.io/rubentalstra/charts/ferroehr \
  --version 6.0.30 -n ferroehr --reuse-values \
...
```shell
FERROEHR_IMAGE=ghcr.io/rubentalstra/ferroehr:4.0.12 \
