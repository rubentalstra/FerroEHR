...
`v4.0.6-rc3`) and the architecture for `<arch>` (`x86_64` or `aarch64`) throughout
...
```bash
gh attestation verify oci://ghcr.io/rubentalstra/ferroehr:4.0.6-rc3 \
  -R rubentalstra/FerroEHR
```
...
```bash
digest=$(docker buildx imagetools inspect ghcr.io/rubentalstra/ferroehr:4.0.6-rc3 \
  | awk '/^Digest:/{print $2}')
