...
conformance: a *class* is not a marketing label a vendor writes down, it is a
verdict a server **earns by measurement** on a stated environment, or does not.
...
— proof-of-concept (`POC`), small (`S`), large (`L`), and regional (`R`). Each
...
commit composition) — never a database backdoor — so what the run reads is exactly
