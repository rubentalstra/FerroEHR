//! The external SUT client: a pure `reqwest` API client with the two
//! credential slots (regular clinical user + ADMIN-role user). It never reaches
//! into the SUT's database — every op is self-contained through the API, so the
//! request path is identical for both benchmark targets (the fairness
//! guarantee). Absorbed from the retired ECC harness.

use async_trait::async_trait;

use crate::sutclient::harness::{
...
pub struct SutClient {
    ...
    /// openEHR base, so this stays `None` for our runs — zero drift.
...
impl SutClient {
    ...
    /// the behaviour of our own server's runs.
    #[must_use]
    ...
    fn credential(&self, slot: AuthSlot) -> Option<&Credential> {
        match slot {
            ...
            // An admin request with no admin slot configured falls back to the
