...
refusal_registry_gate() {
  ...
  local -a registry=(
    ...
    "backup-cronjob.yaml|existingSecret is empty|${base}|--set backup.enabled=true --set backup.clinical.persistentVolumeClaim=clinical-dumps --set backup.demographic.persistentVolumeClaim=demographic-dumps --set backup.linkage.persistentVolumeClaim=linkage-dumps|backup.clinical.existingSecret;BYPASSRLS"
    "backup-cronjob.yaml|existingSecret is empty|${base}|--set backup.enabled=true --set backup.clinical.persistentVolumeClaim=clinical-dumps --set backup.demographic.persistentVolumeClaim=demographic-dumps --set backup.linkage.persistentVolumeClaim=linkage-dumps --set backup.clinical.existingSecret=clinical-backup-dsn|backup.demographic.existingSecret;BYPASSRLS"
    "backup-cronjob.yaml|existingSecret is empty|${base}|--set backup.enabled=true --set backup.clinical.persistentVolumeClaim=clinical-dumps --set backup.demographic.persistentVolumeClaim=demographic-dumps --set backup.linkage.persistentVolumeClaim=linkage-dumps --set backup.clinical.existingSecret=clinical-backup-dsn --set backup.demographic.existingSecret=demographic-backup-dsn|backup.linkage.existingSecret;BYPASSRLS"
  )
