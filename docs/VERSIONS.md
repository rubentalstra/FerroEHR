...
| Item | Pin |
|---|---|
| Rust toolchain | stable 1.98 (1.98.1) |
| MSRV | 1.97 |  <!-- deliberately behind the toolchain: see the MSRV bump policy below -->
| Edition | 2024 |
| Cargo resolver | v3 |
...
| RM (Reference Model) | **1.2.0** (development generation; text + BMM refreshed 2026-07-26 @ `66d3ac45` — asterisk-entity normalization only, zero generated-output change) **+ 1.1.0 released, emitted side by side** (#1942: generation module `v1_1` beside the current `v1_2`; RM 1.1.0's own BMM `includes` names `openehr_base_1.2.0`, so `v1_1` resolves against BASE `v1_2` — the released pairing. PIN-HONESTY CAVEAT: the published 1.1.0 BMM carries post-release content — ITEM_TAG/the tags package post-date the release per the amendment record (reported upstream, #2537; the in-repo stable-profile adjudication is #2538) — so `v1_1` is a faithful serialization of the FILE, not of the release TEXT) | 1.1.0 (29-Sep-2020) | dev | generated → `openehr-rm` |
