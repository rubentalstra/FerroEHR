...
- **Code:** `app/ehrbase-rest/src/negotiate.rs:293` (`respond_rm`), `:270` (`respond`), `:334` (`empty`), `:349` (`json_response`); backend trait returns bare `Value` with no header channel — `app/ehrbase-rest/src/dispatch/ehr.rs:57-164`; `app/ehrbase/src/service/api/ehr.rs` (every method returns `Value`). `grep -niE "etag|location" app/ehrbase-rest/src` finds none.
...
- **Code:** `app/ehrbase-rest/src/dispatch/ehr.rs:67-95` (`ehr_create`/`ehr_create_with_id` always `respond_rm` with the full EHR body), `:114-123` (`ehr_status_update` always returns `ok`=200 with body). `EhrCreateParams.prefer` / `EhrStatusUpdateParams.prefer` (`openehr-its/.../generated/ehr.rs:141,210`) are parsed but never read.
...
- **Code:** `app/ehrbase/src/service/api/ehr.rs:78-85` calls `self.status_version(...)`, which returns `original_version(...)` (`app/ehrbase/src/service/ehr.rs:175-186` → `service/versioned.rs:80` builds an `ORIGINAL_VERSION`). Dispatched via `respond_rm::<EhrStatus>` (`dispatch/ehr.rs:96-104`).
...
- **Code:** `app/ehrbase/src/service/ehr.rs:15-37` (`create_ehr`) only guards `ehr.id` uniqueness (`INSERT ... ON CONFLICT DO NOTHING`); it never checks the supplied EHR_STATUS subject against existing EHRs.
...
- **Code:** `app/ehrbase-rest/src/dispatch/ehr.rs:143-153` dispatches to `backend().versioned_ehr_status_version_get_at_time(p)`, but `app/ehrbase/src/service/api/ehr.rs` never overrides it (the param type isn't even imported), so it inherits the generated `NotImplemented` default → **501**.
...
- **Code:** `app/ehrbase/src/service/contribution.rs:218-226` sets `defining_code.code_string = change_type` where `change_type` is the rubric string; `app/ehrbase/src/service/vobject.rs:54-58` defines `CREATION="creation"`, `MODIFICATION="modification"`, `DELETED="deleted"`.
...
- **Code:** `app/ehrbase/src/service/versioned.rs:80-104` (`original_version`) emits `uid`, `contribution`, `lifecycle_state`, `data` — but no `commit_audit`.
...
- **Code:** `app/ehrbase/src/service/versioned.rs:65-76` (`versioned_object`) emits only `uid` + `owner_id`.
...
- **Code:** `app/ehrbase/src/service/ehr.rs:82-96` (`ehr_summary`) emits `system_id`, `ehr_id`, `ehr_status`, `time_created` — no `ehr_access`.
...
- **Code:** `app/ehrbase-rest/src/dispatch/ehr.rs:60,70,79,89,98,107,117` route EHR/EHR_STATUS through `respond_rm` (honours `Accept: application/xml`) and `optional_rm_value`/`rm_value` (accept XML request bodies).
