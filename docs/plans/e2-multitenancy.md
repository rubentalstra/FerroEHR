...
- [x] 2. Schema: migration — `tenant` table + `tenant_id` on the scoping
...
- [x] 3. Service/wire: TenancyConfig (off by default); tenant resolution
      ...
      precedent).
- [x] 4. Tests: cross-tenant isolation (tenant A cannot read/write/list B's
