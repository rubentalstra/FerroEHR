...
| F-48 | DEFECT (minor, events): multi-instance ordering hazard — `FOR UPDATE SKIP LOCKED` lets a second drainer publish later seq rows first (violates the documented per-EHR ordering); and the row-lock transaction is held across broker publishes including retry/backoff rounds. Single-instance deployments unaffected. DECISION 2026-07-17: the supported posture is ONE drainer instance (the deployed shape; the Helm chart runs one server replica by default) — the documented per-EHR ordering guarantee holds only there; a leader lease (or per-EHR ordering keys) is the registered prerequisite for multi-instance eventing, tracked for the Stage-2 scale-out work. | `extensions/events/publisher.rs` drain path | **S** | ✅ decided (single-drainer posture; multi-instance → Stage-2) |
...
| F-51 | Demographic asymmetries — TRIAGED 2026-07-17: (a) relationship create/update lack the party paths' multimedia-externalization gate — REAL, fix when the demographic wire is next touched (small); (b) relationship `ResourceMeta` lacks `last_modified` — REAL, same batch; (c) `get_party_at_time` answering emptiness (→ 204) for a version that is deleted at that instant is CONSISTENT with the EHR-side at-time convention, and the SM (`i_party.adoc` get_party_at_time — "the Version … current at a_time", error only for a nonexistent versioned object) does not mandate the error for a deleted-at-instant read: deliberate, PORT NOTE it in code at the next touch; (d) the tag routes are target-id-keyed (kind = routing grouping; scope isolation holds) — accepted semantics, an existence/kind gate is optional wire hygiene. | `service/demographic/{relationship,api,versioned,tags}.rs` | (a)(b) **S** open · (c)(d) resolved by triage | ◐ |
