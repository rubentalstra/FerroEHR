...
| `ext` helper functions (`openehr_magnitude`, `openehr_timestamp`, the parsers) | R1 §7.6, §9.2; R2 D3; `ext/0001` | each ends in `EXCEPTION WHEN others THEN RETURN NULL`, which opens a subtransaction on every call, and they are emitted once per candidate row per predicate; `openehr_timestamp` is STABLE (correctly, TimeZone) and so never indexable | **change** | rewrite each helper in whichever form MEASURES fastest over the values it sees — `LANGUAGE sql` where the body folds into the statement, `plpgsql` where it does not — dropping the trap wherever a guard is cheaper, and keeping it only where the trapped cast measures cheapest (the two date/time parsers); the STABLE/IMMUTABLE split stays |
