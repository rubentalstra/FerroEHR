...
| 4 | 12 | Service layer (versioning, contributions, audit) | **in progress (2026-07-05)** | `ehrbase::service` (DI `Backend` seam): EHR/EHR_STATUS/COMPOSITION/DIRECTORY/CONTRIBUTION CRUD on the `node`/`vo_version` store, temporal versioning + time-travel, `contribution_create` (atomic), revision history, committer-from-principal; e2e on PG 18 (5 tests). Open: stored-query + item-tag CRUD, `ehr_get_by_subject`, demographic, template link (P13), AQL (P16) |
