...
We declare conformance of EHRbase-rs, deployed in the SMART on openEHR resource-server posture (docker/sut-smart.yml: smart.enabled, smart.require_smart_scopes, auth.oidc trusting the committed CNF test issuer), to the openEHR CNF 2.0 platform test schedule under the declared capabilities, options and technology profile; every claim derives from the committed machine-verified results.
