...
notes: plan cache (moka, text-keyed) skips parse+lower on repeat text; terminology expansion seam pre-plan; weak content-digest `ETag` on the 200; JSON only (XML Accept → 406); optional `[query].timeout_ms` → 408. GET parameter binds are the spec's NAMED query-string parameters (`Request.md` §Query parameters; `$` prefix tolerated; reserved controls `ehr_id`/`offset`/`fetch`/`q`/`query_parameters` excluded; the JSON-object `query_parameters=` form stays an accepted superset, named wins collisions).
