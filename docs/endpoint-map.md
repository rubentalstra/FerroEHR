...
sql: 2 round trips — SELECT count(*) FROM contribution WHERE ehr_id=$1 (via `versioning::count_contributions`, which first runs the EHR existence probe → 404) as `total`; SELECT c.id, a.time_committed, a.change_type, a.committer#>>'{name}' FROM contribution c JOIN audit a ON a.id=c.audit_id WHERE c.ehr_id=$1 ORDER BY a.time_committed DESC, c.id DESC OFFSET $2 LIMIT $3
notes: OUR OWN EXTENSION — no openEHR spec governs it (the ITS-REST contract defines only the by-uid CONTRIBUTION GET). Session-authenticated (Clinical RBAC class; ABAC Pre-checked on the target EHR, subject-gated like the sibling EHR reads). Response `{ "rows": [ { uid, time_committed, committer, change_type } ], "total" }` newest-first; `offset` default 0, `fetch` default 20 capped at 100. `committer` = the audit committer PARTY_PROXY's `name`; `change_type` = its stored `audit_change_type` code. JSON only (a DTO with no canonical-XML shape → 406 on an XML-only Accept). Unknown EHR → 404.
