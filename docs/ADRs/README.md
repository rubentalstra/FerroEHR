...
| 015 | Fully-integrated multi-tenancy | current | Tenant = logical openEHR system (own system_id); RLS FORCE on tenant-scoped tables; single-tenant default with zero overhead; claim→SET LOCAL resolution; admin tenant CRUD. |
| 016 | FHIR R4 connectors + read façade | current | Mapping-as-data (template↔profile artefacts); inbound via the validated commit path + FEEDER_AUDIT provenance; outbound via outbox events + AQL-backed façade; starter resource set; never a FHIR server. |
| 017 | DV_MULTIMEDIA S3 externalization | current | Threshold offload (off by default), content-addressed sha-256 blobs w/ RM integrity fields, transparent re-inline on read, object_store backend (SeaweedFS dev/test), GC via admin delete. |
