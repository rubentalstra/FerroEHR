...
| Component kit | [`thaw`](https://github.com/thaw-ui/thaw) (Fluent-design) | **git rev `0726a3d`** (main, 2026-05-07 — the W0 fallback, executed 2026-07-17) | Rust → WASM. **Owner decision (2026-07-13): the 0.5 line** (stable 0.4.8 = Leptos 0.7, unusable). **W0 finding:** the only crates.io release, `=0.5.0-beta` (2025-05-03), compiles under `cargo check` and cargo-leptos dev builds (which pass `--cfg erase_components`) but **fails plain `cargo build`/`cargo test` codegen on rustc 1.96** — its deep `Layout` view types blow the layout-recursion depth; thaw main fixes it with `#![recursion_limit = "256"]` (its `lib.rs` line 1). So the pin is the main rev above (deny.toml `allow-git` carries the dated exception); re-pin to crates.io when thaw 0.5 stable ships. (Leptonic is stale — **do not use**.) |
