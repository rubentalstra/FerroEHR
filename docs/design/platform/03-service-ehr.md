...
| G-3 | RM ehr master04 §Folders; `EHR` `Folders_valid`/`Directory_in_folders`; arch A6 | MED | **quarantine / PORT NOTE** — read side is multi-hierarchy (`live_folder_hierarchies` `ehr.rs:553`); write side single-slot by design (`directory.rs:30`). Owned by **WORKLIST W-6** (`claude/w3b-arch-gaps`) — cross-ref only, do not re-plan. |
