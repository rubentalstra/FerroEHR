...
A pure-Rust, **openEHR-spec-conformant** CDR (ITS-REST 1.0.3 + AQL 1.1) in a single root Cargo workspace, with greenfield PG18-native internals. The openEHR spec + serialization + REST-contract layer is **generated** from the official machine-readable specs; the application is **modern idiomatic Rust of our own design on top of the generated crates**: our own storage, versioning, and AQL engine, validated by the openEHR CNF conformance suite — EHRbase is prior art, not an oracle. The forward product roadmap is the root `ROADMAP.md`; running plans + the single open-items tracker live in `docs/plans/` (`WORKLIST.md`). ROADMAP.md + the plan files + `docs/PROGRESS.md` are the authoritative record. **There is no ADR/design-doc layer (owner rulings 2026-07-16 + 2026-07-17): decisions live in THIS file, `docs/architecture.md`, and the code; a plan/design markdown is DELETED in the PR that implements it; the vendored specs are the only doc oracle.**
...
- **Pipeline:** vendored `*.bmm.json` (in `tools/openehr-codegen/vendor/bmm/`) → `openehr-lang::bmm` (loader) → `openehr-codegen` (emitter) → the spec crates. `openehr-derive`'s `#[derive(OpenEhrType)]` supplies canonical-JSON `_type` (de)serialization on the emitted types.
...
5. Commit on a conventional-type branch (`feat/…`, `fix/…`, `chore/…` — see the branch hard rule below) with a descriptive subject.
...
- **Branches use the industry-standard conventional types (owner hard rule, 2026-07-19 — the former `claude/*` scheme is RETIRED, never create a new `claude/*` branch):** `<type>/<kebab-case-slug>` with `type` ∈ `feat` | `fix` | `chore` | `docs` | `refactor` | `perf` | `test` | `ci` | `build` | `release` (mirroring the Conventional Commits type set) — e.g. `feat/adl2`, `fix/tenant-guc-pool-stamp`, `chore/dep-bumps`. Pick the type by the dominant change; a worklist-row phase branch is normally `feat/<row-slug>`. Existing historical `claude/*` branches/PR links stay as recorded facts. Never force-push `main`/`develop`. Never delete `docs/plans/WORKLIST.md` or `docs/plans/README.md` (the single tracker + the lifecycle guide); implemented plan files are DELETED in the PR that lands them, with the close recorded in `docs/PROGRESS.md`. (Enforced by the `block_dangerous` hook's force-push allowlist + review.)
