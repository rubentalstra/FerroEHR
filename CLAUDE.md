...
- `docs/` — plans (`docs/plans/`: deep working plans, linked from their tracker issues), VERSIONS, architecture, postgres-features, `endpoint-map.md` (every endpoint traced to its SQL), conformance (generated reports + measurement artifacts); the forward product roadmap is the root `ROADMAP.md`. **`docs/specs/openehr/` — the vendored openEHR spec text + CNF test schedule (the conformance oracle; see its README + `/spec-lookup`).**
