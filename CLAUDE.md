...
OpenAPI/Swagger: `utoipa` 5, `utoipa-axum` 5, `utoipa-swagger-ui` 9 (serves Swagger UI). Note (owner rule 2026-07-17): the vendored ITS-REST OpenAPI is the **codegen input** for the generated `openehr-its` contract (`emit-rest`) ONLY — it is **NOT a behavioural oracle** (owner ruling 2026-07-24: the OAS is **stalled**; the ITS-REST **docs text**, `docs/specs/openehr/ITS-REST/`, is the conformance oracle, and where the OAS and the docs text disagree, the docs text wins). It is NEVER imported into or served by `ehrbase-rest`. The server serves ONLY its own natively generated OpenAPI (`#[utoipa::path]` on every handler, composed in `ehrbase-rest::extensions::openapi`; owner hard rule: serve only what we generate). Any surface change updates our `#[utoipa::path]` declarations in the same PR.
