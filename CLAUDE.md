...
- `app/*` — the application (ADR-010): `ehrbase` (the binary), `ehrbase-rest`, `ehrbase-compat`, `ehrbase-audit`, `ehrbase-authz`, `ehrbase-signing`, and the `ehrbase-sm` native-API crate (SM-1). `tools/*` — dev/verification tooling, **not part of the app**: `conformance` (the ECC runner) and `benchmark`. Workspace `members = ["crates/*", "app/*", "tools/*"]`. The service layer follows the openEHR **SM Platform Service Model** (ADR-010; design set `docs/design/sm-platform/`).
