...
- `ehrbase-rest`, `ehrbase-compat`, `ehrbase` (+ `ehrbase-audit`, `ehrbase-authz`, `ehrbase-signing`, `ehrbase-conformance`, `ehrbase-bench`, and the ADR-010 `ehrbase-sm` native-API crate) — the application (built idiomatically; `ehrbase` is the binary). **Layout (ADR-010): the `ehrbase-*` crates live under `app/*`; the spec layer stays under `crates/*`** (`members = ["crates/*", "app/*"]`; the physical move is SM-1's opening task — until it lands, everything is still under `crates/*`). The service layer follows the openEHR **SM Platform Service Model** (ADR-010; design set `docs/design/sm-platform/`).
