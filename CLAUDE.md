...
- **NOT generated** (hand-written): `openehr-term` (terminology bundle + XML assets + access logic — BMM only has ~6 interface classes), `openehr-query` (AQL lexer/parser/AST), `openehr-adl` (the ADL 2.4 engine), `openehr-its::flat` (Simplified Formats — the hand-written `flat` module of the otherwise mixed `openehr-its` crate), and the `ehrbase-*` application crates (idiomatic Rust of our own design on the generated crates).
...
**Relationships (native GitHub structure, not prose — the full policy + the ONLY sanctioned commands live in `.claude/rules/issue-relationships.md`):** GitHub has four native issue edges; we treat them as first-class tracker structure and set every one with **`scripts/gh-rel.sh`** (never hand-typed `gh api` — the write endpoints take the issue's *database id*, not its `#number`, and `gh` has no native subcommand; the helper resolves the id and fails loud). **Sub-issues** (`scripts/gh-rel.sh parent <child> <parent>`) decompose a genuinely multi-part issue into individually-closeable children whose progress rolls up (≤100 children, ≤8 levels) — but they do NOT stand in for release grouping: **milestones remain the release spine (no per-release "epic" parent issues** — owner ruling 2026-07-24). **Blocked-by / blocking** (`scripts/gh-rel.sh blocked-by`/`blocking`) express real in-repo sequencing (≤50 per direction); an *upstream* wait stays the `blocked-upstream` label (you cannot be `blocked_by` a Jira ticket, and there is no in-repo issue to point at). **Security alerts** link a code-scanning alert to an issue — UI-only, no API (enabled by `.github/workflows/codeql.yml`; the manual flow is documented in the rule file). The SessionStart dump + `/phase-status` surface `{k/n}` progress, `child-of #x`, and open `BLOCKED-by`/`blocks` inline; `/next-task` skips blocked issues; `/phase-done` refuses to close a parent with open children. **An edge lives in exactly ONE place — its native panel, NEVER the issue body:** a parent's body never lists its sub-issues (the Sub-issues panel + `{k/n}` bar are canonical; a body list shows the children twice and rots on the next add/close/reparent), an issue's body never lists its blockers, and a parent's exit criteria are outcomes, not a checkbox roll-call of children (rule §No duplication).
...
OpenAPI/Swagger: `utoipa` 5, `utoipa-axum` 5, `utoipa-swagger-ui` 9 (serves Swagger UI). Note (owner rule 2026-07-17): the vendored ITS-REST OpenAPI is the **codegen input** for the generated `openehr-its` contract (`emit-rest`) ONLY — and a **subordinate wire source** (owner rulings 2026-07-24 + 2026-07-28): the ITS-REST **docs text**, `docs/specs/openehr/ITS-REST/`, is the conformance oracle and WINS wherever the two disagree; where the docs text is SILENT, the released OAS grounds the expectation (the release's own overview `Specifications.md` presents the OAS files as its computable artifacts); only both-silent goes to the ambiguity register. It is NEVER imported into or served by `ehrbase-rest`. The server serves ONLY its own natively generated OpenAPI (`#[utoipa::path]` on every handler, composed in `ehrbase-rest::extensions::openapi`; owner hard rule: serve only what we generate). Any surface change updates our `#[utoipa::path]` declarations in the same PR.
