...
//   confidence: medium
//   todos: 3
//   note: abstract class, no attributes beyond value: DV_PARSABLE and three abstract functions -> pure trait (ADR-001 §1/§3 boundary case: no shared state struct needed since DATA_VALUE's own state is not yet landed and this class adds no further attributes). All three functions require an HL7v3 PIVL/EIVL/GTS syntax parser not yet designed (distinct from the ISO-8601-specific jiff bridging plan). Closed-enum decision for the two concrete descendants deferred to whichever call site needs it. P4: no serde derive needed — a pure trait with no attributes of its own has nothing to annotate; its two implementors (DvPeriodicTimeSpecification, DvGeneralTimeSpecification) each carry the real derive.
// ─────────────────────────────────────────────
