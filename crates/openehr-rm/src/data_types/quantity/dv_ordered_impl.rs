//! Hand-written RM spec functions (ADR-003) for the `DV_ORDERED` /
...
//!
//! PORT NOTE (ADR-008): the *indexed* query-path realisation of these ordering
...
//
// The inputs are first checked with the `crate::validate` ISO-8601 subset
...
pub(crate) fn push_normal_range_consistency<T: OrderedLimit>(
        ...
        && let Some(in_range) = range.has(value)
    {
        ...
        if !(is_n ^ !in_range) {
            out.push(crate::validate::invariant_failed(
