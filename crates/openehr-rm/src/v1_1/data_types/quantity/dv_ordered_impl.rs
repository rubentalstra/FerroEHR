...
//
// The inputs are first checked with the `crate::v1_1::validate` ISO-8601 subset
...
/// Format a (days-since-`0001-01-01`, seconds-in-day) pair back into an
/// extended ISO-8601 date-time string `YYYY-MM-DDThh:mm:ss[.fff]` (fractional
...
/// Decompose an ISO-8601 date-time into `(days since 0001-01-01, seconds in
/// day, timezone suffix)` **without** normalising to UTC — used for date-time ±
/// duration arithmetic that must preserve the value's own offset.
#[must_use]
pub(crate) fn iso_date_time_parts(s: &str) -> Option<(i64, f64, String)> {
    if !is_valid_iso_date_time(s) {
        return None;
    }
    match s.split_once('T') {
        Some((date, time)) => {
            let days = iso_date_magnitude_days(date)?;
            let tz_start = {
                // The timezone suffix is `Z` or the last `+`/`-` in the time part.
                if time.ends_with('Z') {
                    Some(time.len() - 1)
                } else {
                    time.rfind(['+', '-'])
        ...
        None => Some((iso_date_magnitude_days(s)?, 0.0, String::new())),
    }
}
...
pub(crate) fn push_normal_range_consistency<T: OrderedLimit>(
    ...
    rm_type: &str,
    normal_status: Option<&crate::v1_1::data_types::text::code_phrase::CodePhrase>,
    normal_range: Option<&crate::v1_1::data_types::quantity::dv_interval::DvInterval<T>>,
    value: &T,
) {
        ...
        && let Some(in_range) = range.has(value)
    {
        ...
        if !(is_n ^ !in_range) {
            out.push(crate::v1_1::validate::invariant_failed(
