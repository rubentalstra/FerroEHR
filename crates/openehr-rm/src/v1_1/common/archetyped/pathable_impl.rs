// @generated-from-template templates/openehr-rm/common/archetyped/pathable_impl.rs — DO NOT EDIT; edit the source and re-run `openehr-codegen -- emit`.
...
//! once in [`crate::v1_1::paths`], over the canonical-JSON value tree, and a
...
//!   root-anchored lookup ([`crate::v1_1::paths::parent_of`]) — it needs the
...
//!   ([`crate::v1_1::paths::path_of_item`]) locates that node by identity;
...
use crate::v1_1::common::archetyped::pathable::Pathable;
use crate::v1_1::paths::{RmPath, TypedPathError, canonical_value};
...
impl Pathable {
    ...
    pub fn item_at_path(&self, a_path: &str) -> Result<Option<Value>, TypedPathError> {
        ...
        Ok(crate::v1_1::paths::item_at_path(&root, &path).cloned())
    }
    ...
    pub fn items_at_path(&self, a_path: &str) -> Result<Vec<Value>, TypedPathError> {
        let path: RmPath = a_path.parse()?;
        let root = canonical_value(self)?;
        Ok(crate::v1_1::paths::items_at_path(&root, &path)
            .into_iter()
            .cloned()
            .collect())
    }
