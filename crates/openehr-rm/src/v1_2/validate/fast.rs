...
pub(super) fn try_validate(ty: &str, value: &Value, out: &mut Vec<InvariantViolation>) -> bool {
    ...
    let Some(spec) = crate::v1_2::model::class(ty) else {
        return false;
    };
...
fn class_slot_conforms(v: &Value, declared: &str, shallow: bool) -> bool {
    ...
    let Some(slot) = crate::v1_2::model::class(declared) else {
        return false;
    };
...
fn run_invariants(ty: &str, obj: &Map<String, Value>, out: &mut Vec<InvariantViolation>) -> bool {
    match ty {
        ...
        "DV_URI" => {
            ...
            crate::v1_2::data_types::uri::dv_uri_impl::push_dv_uri_invariants(value, out);
        }
        "DV_EHR_URI" => {
            let Some(value) = str_of(obj, "value") else {
                return false;
            };
            crate::v1_2::data_types::uri::dv_ehr_uri_impl::push_dv_ehr_uri_invariants(value, out);
        }
        "DV_IDENTIFIER" => {
        ...
        "ELEMENT" => {
            ...
            crate::v1_2::data_structures::representation::element_impl::push_element_invariants(
                present(obj, "value"),
