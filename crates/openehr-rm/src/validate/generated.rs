...
//! - `REVISION_HISTORY_ITEM.Audit_valid`: `not audits.is_empty`: satisfied by construction. A REVISION_HISTORY is a read-only derived resource — no operation accepts one as a request body — and every builder of an item pushes that version's commit audit as the first audit before any attestation, which is what §Attributes means by "there will always be at least one commit audit". (docs/specs/openehr/RM/docs/UML/classes/org.openehr.rm.common.revision_history_item.adoc §Invariants).
