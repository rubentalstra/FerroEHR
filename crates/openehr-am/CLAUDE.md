...
The Archetype Model, both versions as separate generation modules: `v1_4`
(ADL 1.4 / OPT 1.4) and `v2_4` (ADL 2 / AOM 2.4). **Generated from the
...
- Keep `v1_4` and `v2_4` strictly parallel — never blend the two models or
...
- Template/OPT ingestion and WebTemplate building live in the application
  ...
  `crates/openehr-adl`.
- Spec pins are per generation: the emitted `Generation` enum (`V1_4`/`V2_4`, `V2_4` is `#[default]`) + each generation module's `SPEC_VERSION` — no crate-level pin; the package version is the
