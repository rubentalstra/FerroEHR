# `openehr-am` — AM 1.4.0 + 2.4.0 (GENERATED)

The Archetype Model, both versions as separate generation modules: `v1_4`
(ADL 1.4 / OPT 1.4) and `v2_4` (ADL 2 / AOM 2.4). **Generated from the
vendored BMM** by `openehr-codegen -- emit`.

- **NEVER hand-edit a file with a `// @generated … DO NOT EDIT` header.**
  Change the emitter and regenerate (`/regen-codegen`); `codegen-drift` CI
  guards it.
- Hand-written constraint semantics live ONLY in `*_impl.rs` siblings with
  AM spec citations (`docs/specs/openehr/AM/docs/` — ADL1.4, ADL2, AOM1.4,
  AOM2). AOM2 validity codes (VCOC, VATID, VCORM, …) are the error
  vocabulary for template-ingestion validation — keep codes exact.
- Keep `v1_4` and `v2_4` strictly parallel — never blend the two models or
  "share" a type across them outside what the BMM defines.
- Template/OPT ingestion and WebTemplate building live in the application
  (`app/ferroehr`), not here; this crate is the model only. The ADL2/AOM2 text
  engine (parser, validation, flattening, OPT2, 1.4→2 conversion) is
  `crates/openehr-adl`.
- Spec pins are per generation: the emitted `Generation` enum (`V1_4`/`V2_4`, `CURRENT = V2_4`) + each generation module's `SPEC_VERSION` — no crate-level pin; the package version is the
  crate's own SemVer line (`.claude/rules/crates-publishing.md`). Gates:
  the crate stays lib-clippy-clean by construction — fix the emitter, never the
  output.

- `src/json_serde.rs` is GENERATED by `emit-json` — the canonical-JSON
  `serde::Serialize`/`Deserialize` impls for this crate's types, over the shared
  hand-written runtime `openehr_base::serde_support`. Never hand-edit it; change
  `tools/openehr-codegen/src/render/emit_json.rs` and regenerate.
