...
//! The openEHR `AUTHORED_ARCHETYPE` spec class, generated from the vendored BMM
//! meta-model.
...
use crate::v2_4::aom2::archetype::template::Template;
...
/// Polymorphic slot of `AUTHORED_ARCHETYPE`, dispatched on each payload's `_type`.
///
/// Root object of a standalone, authored archetype, including all meta-data, description, other identifiers and lifecycle.
#[doc(alias = "AUTHORED_ARCHETYPE")]
#[derive(Debug, Clone, PartialEq)]
pub enum AuthoredArchetype {
    /// The `OPERATIONAL_TEMPLATE` subtype of `AUTHORED_ARCHETYPE`.
    OperationalTemplate(Box<OperationalTemplate>),
    /// The `TEMPLATE` subtype of `AUTHORED_ARCHETYPE`.
    Template(Box<Template>),
    /// An instance of `AUTHORED_ARCHETYPE` itself (its own, least-rich form).
