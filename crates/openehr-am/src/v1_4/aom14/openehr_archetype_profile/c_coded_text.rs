// @generated by openehr-codegen from BMM (`C_CODED_TEXT`) — DO NOT EDIT.
// SPDX-FileCopyrightText: Ruben Talstra
// SPDX-FileCopyrightText: openEHR Foundation
// SPDX-License-Identifier: BUSL-1.1 AND Apache-2.0

//! The openEHR `C_CODED_TEXT` spec class, generated from the vendored BMM
//! meta-model.

#![expect(
    clippy::disallowed_types,
    reason = "adjudicated free-form JSON slots: serde_json::Value is workspace-banned (#1694); a generated carrier exists only where the spec leaves the slot open, and each adjudicated field's NOTE names its citation"
)]
use openehr_base::v1_3::foundation_types::interval::interval::Interval;

/// Constrainer class for Coded text data.
#[doc(alias = "C_CODED_TEXT")]
#[derive(Debug, Clone, PartialEq)]
pub struct CCodedText {
    // inherited: C_OBJECT
    /// Reference model type that this node corresponds to.
    pub rm_type_name: String,
    /// Occurrences of this object node in the data, under the owning attribute. Upper limit can only be greater than 1 if owning attribute has a cardinality of more than 1).
    pub occurrences: Interval<i32>,
    /// Semantic identifier of this node, used to distinguish sibling nodes.
    ///
    /// All nodes must have a node_id; for nodes under a container C_ATTRIBUTE, the id must be an id-code must be defined in the archetype terminology. For valid structures, all node ids are at-codes.
    /// For C_PRIMITIVE_OBJECTs, it will have the special value Primitive_node_id.
    pub node_id: String,

    // inherited: C_DEFINED_OBJECT
    /// Value to be assumed if none sent in data.
    pub assumed_value: Option<serde_json::Value>,
    /// Terminology identifier.
    pub terminology: String,
    /// Optional list of codes from the terminology. No list means any code from the terminology is allowed.
    pub code_list: Option<Vec<String>>,
    /// The `reference` attribute of openEHR `C_CODED_TEXT` (the vendored BMM carries no documentation for it).
    pub reference: Option<String>,
}
