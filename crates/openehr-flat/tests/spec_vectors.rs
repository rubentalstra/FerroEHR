...
//!
//! - **FlatKey parse/print** (`openehr_flat::path::FlatKey`) round-trips
...
//!   `openehr_flat::convert::{flat_to_structured, structured_to_flat}` and
//!   `openehr_flat::sim::flat::{parse_flat, emit_flat}`.
//! - **Reject rules** via `openehr_flat::convert::composition_from_flat`
//!   against a minimal hand-built Web Template (unknown path/suffix,
//!   malformed keys, `|raw` without `_type`, `|other` conflicts/closed set).
//! - **Mandatory context** via `openehr_flat::validation::validate_context`
...
use openehr_flat::convert::{composition_from_flat, flat_to_structured, structured_to_flat};
use openehr_flat::error::FlatError;
use openehr_flat::path::FlatKey;
use openehr_flat::sim::flat::{emit_flat, parse_flat};
use openehr_flat::validation::{ValidationKind, validate_context, validate_flat_other};
