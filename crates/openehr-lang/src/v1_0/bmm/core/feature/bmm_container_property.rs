...
/// Polymorphic slot of `BMM_CONTAINER_PROPERTY`, dispatched on each payload's `_type`.
///
/// Meta-type of for properties of linear container type, such as List\<T\> etc.
#[doc(alias = "BMM_CONTAINER_PROPERTY")]
#[derive(Debug, Clone, PartialEq)]
pub enum BmmContainerProperty {
    /// The `BMM_INDEXED_CONTAINER_PROPERTY` subtype of `BMM_CONTAINER_PROPERTY`.
    BmmIndexedContainerProperty(BmmIndexedContainerProperty),
    /// An instance of `BMM_CONTAINER_PROPERTY` itself (its own, least-rich form).
