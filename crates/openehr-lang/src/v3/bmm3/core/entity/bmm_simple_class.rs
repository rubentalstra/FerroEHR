...
/// Polymorphic slot of `BMM_SIMPLE_CLASS`, dispatched on each payload's `_type`.
///
/// Definition of a simple class, i.e. a class that has no generic parameters and is 1:1 with the type it generates.
#[doc(alias = "BMM_SIMPLE_CLASS")]
#[derive(Debug, Clone, PartialEq)]
pub enum BmmSimpleClass {
    /// The `BMM_ENUMERATION` subtype of `BMM_SIMPLE_CLASS`.
    BmmEnumeration(BmmEnumeration),
    /// An instance of `BMM_SIMPLE_CLASS` itself (its own, least-rich form).
    BmmSimpleClass(BmmSimpleClassData),
}
