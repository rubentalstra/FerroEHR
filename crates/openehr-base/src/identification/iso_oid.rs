...
/// `value` attribute directly into this struct's JSON object, so an
/// `IsoOid` serializes as the canonical `{"_type": "ISO_OID", "value":
/// "..."}` UID shape (ADR-002 self-tag) rather than
/// `{"uid": {"value": "..."}}`.
#[derive(
    Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash, serde::Serialize, serde::Deserialize,
)]
pub struct IsoOid {
    /// Canonical `_type` discriminator (`"ISO_OID"`), always serialized
...
impl TypeName for IsoOid {
    const NAME: &'static str = "ISO_OID";
}
...
//   confidence: high
//   todos: 0
//   note: pure UID subtype with no added attributes/functions; grammar-form validation (dot-separated numbers) not yet enforced. P4/ADR-002: self-tags via TypeTag<Self> first field (no prior TYPE_NAME const existed in this file; NAME taken from the ITS-JSON schema string "ISO_OID").
// ─────────────────────────────────────────────
