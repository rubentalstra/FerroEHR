...
/// both [`VersionStatus::symbol`] and the `#[serde(rename = "...")]` on
/// each variant below.
///
/// P4 update: as with `ValidityKind` in this same package, `openehr-base`
/// now depends on `serde` (`PORT_MASTER_PLAN.md` §10); each variant is
/// tagged with the spec's own lower-case/snake_case wire form (matching
/// `symbol()` exactly), not an uppercase class-style `_type` tag —
/// `VERSION_STATUS` is an enumeration value, not an RM/AM class name.
...
pub enum VersionStatus {
    ...
    #[serde(rename = "release_candidate")]
    ReleaseCandidate,
