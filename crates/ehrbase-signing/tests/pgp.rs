...
use ehrbase_signing::{KeyError, Mode, Signer, SigningConfig, SigningError, Verdict, VerifyOnRead};
...
fn generate_key(passphrase: Option<&str>) -> String {
    ...
    builder
        ...
        .can_sign(true)
        .primary_user_id("ehrbase-signing test <test@ehrbase.local>".into());
...
fn temp_file(content: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("ehrbase-signing-{}-{n}.asc", std::process::id()));
