//! Read-time verification (design §3.5). We do not persist a signature's
//! provenance (the migration adds only `signature text`), so [`verify`]
//! classifies by **format**:
//!
...
//!
//! PORT NOTE (design §3.5): "client-supplied signatures are exempt from
//! digest/pgp recomputation" is realised by this format classification: a
//! foreign signature (neither our `sha256:` digest nor a PGP signature we hold a
//! key for) is [`Verdict::ClientForeign`] and never a failure. Only our own
//! output formats (`sha256:` in any mode, PGP in `pgp` mode) are judged.
...
pub(crate) fn verify(mode: &SignerMode, canonical: &str, signature: &str) -> Verdict {
    ...
    if signature.trim_start().starts_with(PGP_ARMOR) {
        return match mode {
            ...
            // signature — structural armor check only (§3.5).
            SignerMode::Digest => {
