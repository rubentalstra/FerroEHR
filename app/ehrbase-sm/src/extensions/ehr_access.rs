...
/// The native-API extension exposing the current `EHR_ACCESS` scheme settings.
///
/// The SM defines no `I_EHR_ACCESS` interface (authorisation is out of band —
/// SM `openehr_platform/master02-overview.adoc`), so this is our own extension
/// (`no openEHR spec governs this adapter — our own extension`,
/// `docs/design/ehr-access-scheme.md`). The platform component reads the
/// current `EHR_ACCESS` version of the EHR through the normal versioned-object
/// path and parses its `settings`; the protocol adapter enforces them.
#[async_trait]
pub trait EhrAccessAdapter: Send + Sync {
    /// The current `EHR_ACCESS.settings` of `ehr_id` parsed as this scheme, or
    /// `None` when the EHR carries no settings, uses another scheme, or does not
    /// exist — all default-open.
