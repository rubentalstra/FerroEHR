...
-- (ADR-015 / E2).
--
-- Append-only on the ADR-013 baseline (0001) + eventing (0002/0003). Adds:
--   * `tenant` — the registry: one row per logical openEHR system (its own
--     system_id, ADR-015 §1), seeded with the reserved DEFAULT tenant;
...
--     USING/WITH CHECK `tenant_id = ext.current_tenant_id()` (ADR-015 §2).
-- Runs with search_path = ehr, ext.
--
-- Single-tenant default, ZERO behaviour change (ADR-015 §3): tenancy is OFF by
-- default; no session sets ehrbase.tenant_id, so every row is the reserved
...
-- integration test connecting as a non-superuser role member of ehrbase_app.
--
-- Baseline discipline: named constraints (pk_/uq_/fk_/ck_), COMMENT ON
-- everything, role-guarded grants.
--
-- INDEXING (ADR-015 §Consequences — "measured at P20"): no dedicated tenant_id
...
-- multi-tenant mode", ADR-015 §Consequences).
--
-- PORT NOTE (ADR-015 §2 scope): the pre-existing service-wide UNIQUE
...
-- (what ADR-015 §2 specifies as engine-enforced isolation); making those keys
...
CREATE TABLE tenant (
    ...
    name       text NOT NULL,
    -- The tenant's own logical-system id (ADR-015 §1): per-tenant version
...
COMMENT ON TABLE tenant IS 'ADR-015 §1 tenant registry: one row per logical openEHR system (its own system_id). NOT itself RLS-scoped — it is the registry the tenant-resolution middleware resolves a claim/header against. The reserved default tenant (nil uuid) owns all rows when tenancy is OFF.';
COMMENT ON COLUMN tenant.system_id IS 'The tenant''s own logical-system id (ADR-015 §1): stamped into version ids / audits / EHR.system_id for this tenant when tenancy is ON.';

-- The reserved DEFAULT tenant (ADR-015 §3): fixed nil uuid, owns every row
...
-- 'ehrbase-rs.local'); with tenancy OFF the service uses its own configured
...
VALUES ('00000000-0000-0000-0000-000000000000', 'default', 'ehrbase-rs.local');

-- ── tenant_id scoping + RLS FORCE (ADR-015 §2) ───────────────────────────────
...
BEGIN
    FOREACH t IN ARRAY scoped_tables LOOP
        ...
        EXECUTE format('COMMENT ON COLUMN %I.tenant_id IS %L', t,
            'ADR-015 §2: owning tenant (FK → tenant). DEFAULT ext.current_tenant_id() '
        ...
        -- FORCE so the table OWNER is filtered too (ADR-015 §2); superusers /
...
BEGIN
    IF EXISTS (SELECT FROM pg_roles WHERE rolname = 'ehrbase_app') THEN
        GRANT SELECT, INSERT, UPDATE, DELETE ON tenant TO ehrbase_app;
        GRANT SELECT ON tenant TO ehrbase_reader;
    ELSE
