...
//!
//! Env keys are the `EHRBASE_SIGNING_`-prefixed set (design
//! `docs/design/version-signing.md` §5). Loading mirrors the other configs:
...
/// The signing mode (design §3.2): a data-integrity digest, or an `OpenPGP`
/// (RFC 4880) digital signature.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
pub enum Mode {
    /// `radix-64(SHA-256(canonical_form))` — a data-integrity check, no key
    /// management (the STANDARD "Signing" capability met out of the box).
    #[default]
    Digest,
...
/// Recompute-and-compare policy at read/reassembly time (design §3.5).
#[derive(Debug, Clone, Copy, PartialEq, Eq, Deserialize, Default)]
#[serde(rename_all = "lowercase")]
pub enum VerifyOnRead {
    /// Serve the stored signature untouched (the spec's model: a stored fact).
    #[default]
    Off,
...
/// value is valid: signing **on**, `digest` mode, verification off (design §3.4).
