//! `OpenPGP` key loading + the rPGP detached-signature primitives (design §4.2).
//!
//! Spec: RM common §"Digital Signature" — the signature "is generated according
//! to the `openPGP` standard (IETF RFC 4880)". PORT NOTE (design §3.2): an RFC
//! 4880 signature internally hashes the signed data, so a detached signature
//! over the `canonical_form` bytes is the standard-conformant realisation of the
//! spec's "digital signature can be created from the hash".
...
/// mode; RM common §"Digital Signature" leaves the algorithm to the format).
...
impl PgpKey {
    ...
    /// Load an armored RFC 4880 secret key from an in-memory string (the file
    /// path indirection of [`load`](Self::load) removed — used by tests).
    ///
    /// # Errors
    ...
    pub fn from_armored(armored: &str, passphrase: Option<&str>) -> Result<Self, KeyError> {
        ...
        // and it can actually sign (design §4.2 boot validation).
        key.sign(b"ehrbase-signing boot check")
