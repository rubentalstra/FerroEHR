...
impl EhrbaseService {
    ...
    pub(super) async fn list_subscriptions(&self) -> Result<Vec<Value>, ServiceError> {
        let rows = sqlx::query(
    ...
    pub(super) async fn get_subscription(&self, id: Uuid) -> Result<Value, ServiceError> {
        let row = sqlx::query(
            "SELECT id, name, kind, change_type, template_id, archetype, enabled, created_at \
    ...
    pub(super) async fn create_subscription(&self, body: &Value) -> Result<Value, ServiceError> {
