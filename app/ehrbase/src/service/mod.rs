...
//! concrete [`EhrbaseService`] methods, one folder per SM chapter (vendored
//! SM spec `docs/specs/openehr/SM/docs/openehr_platform/`).
//!
//! Each chapter folder owns its domain logic as inherent `EhrbaseService`
//! methods; the protocol adapter (`ehrbase-rest`) calls them directly.
