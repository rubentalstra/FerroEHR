...
//! (it is the registry every tenant's isolation is defined against), so these
//! queries need no session tenant context — and resolution can run before the
//! request's tenant scope is established (no chicken-and-egg).
//!
...
impl EhrbaseService {
    ...
    pub(super) async fn list_tenants(&self) -> Result<Vec<Value>, ServiceError> {
        let rows = sqlx::query(
    ...
    pub(super) async fn get_tenant(&self, id: Uuid) -> Result<Value, ServiceError> {
    ...
    pub(super) async fn create_tenant(&self, body: &Value) -> Result<Value, ServiceError> {
    ...
    pub(super) async fn resolve_tenant(
        &self,
        key: &str,
    ) -> Result<Option<TenantContext>, ServiceError> {
