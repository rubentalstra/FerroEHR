...
impl DefinitionAdapter for EhrbaseService {
    ...
    async fn template_adl14_list(
        ...
        // `parameters/query/filter_template_id.yaml`). Filter + paginate the
        // stored template descriptors here.
        Ok(filter_templates(
    ...
    async fn template_adl2_list(
        ...
        // The ADL2 twin of `template_adl14_list`
        // (`operations/definition_template_adl2_list.yaml`). The store yields
        // `{template_id, created_timestamp}` — `concept` is not extracted (no
        // cADL source parser; PORT NOTE on `adl2_template_list`), so the
        // `concept` filter matches nothing when supplied; `template_id`/`version`
        // globs + `offset`/`fetch` are honoured over the full set here.
        Ok(filter_templates(
    ...
    async fn query_store(
        ...
        // default `AQL`, case-insensitive; `parameters/query/query_type.yaml`).
        // The build can only validate + store AQL, so a non-AQL formalism is an
        // honest *unsupported-formalism* reject (a distinct `400`, not a blanket
        // "invalid AQL") — the store then only ever holds AQL and the descriptor
        // reflects it (`operations/definition_query_store.yaml`, response set
        // `200/400`). AQL bodies fall through to the AQL syntactic check in
        // `store_query` (`400` on a syntactically invalid body).
...
/// `master02-overview.adoc` §List Handling). An empty filter + `Page::all` is
/// the full set.
...
/// (`parameters/query/query_type.yaml`; `master04-definition_package.adoc`,
/// which sanctions "any other string value" that we reject).
