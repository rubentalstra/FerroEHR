...
impl AdminService for EhrbaseService {
    ...
    async fn physical_party_delete(&self, a_party_id: String) -> Result<(), ApiError> {
        Ok(self
...
impl AdminArchive for EhrbaseService {
    async fn archive_ehrs(&self, ehr_ids: Vec<String>) -> Result<(), ApiError> {
        let ids = ehr_ids
            .iter()
            .map(|s| parse_uuid(s, "EHR"))
            .collect::<Result<Vec<_>, _>>()?;
    ...
    async fn archive_parties(&self, party_ids: Vec<String>) -> Result<(), ApiError> {
        let ids = party_ids
            .iter()
...
fn parse_range(range: StatTimeRange) -> Result<(Option<String>, Option<String>), ApiError> {
...
fn parse_bound(bound: Option<String>) -> Result<Option<String>, ApiError> {
    match bound {
        None => Ok(None),
        Some(raw) => {
            let ts: jiff::Timestamp = raw
                .parse()
                .map_err(|_| ApiError::BadRequest(format!("invalid ISO 8601 date-time: {raw}")))?;
