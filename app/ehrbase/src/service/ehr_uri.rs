...
//! canonical-JSON content, over the existing versioned-object read surface.
//!
...
//! [`resolve_ehr_uri`] enforces the `item_at_path` precondition `path_unique`,
//! while [`resolve_ehr_uri_items`] returns every match (`items_at_path`).

use openehr_rm::paths::{EhrUri, TopLevelLocator, VersionLocator};
use serde_json::Value;
use uuid::Uuid;

use super::version_id::{self, TreeId};
...
impl EhrbaseService {
    ...
    /// top-level object; with no locator it is the EHR summary object.
    ///
    /// # Errors
    ///
    ...
    pub async fn resolve_ehr_uri_items(&self, uri: &EhrUri) -> Result<Vec<Value>, ServiceError> {
        ...
        // surface the EHR summary object.
...
fn resolve_object_ref(object: &VersionLocator) -> Result<(Uuid, Option<TreeId>), ServiceError> {
    match object {
        ...
        VersionLocator::Version(ovid) => {
            let (vo_id, tree) = version_id::components(ovid)?;
            Ok((vo_id, Some(tree)))
        }
    }
}
