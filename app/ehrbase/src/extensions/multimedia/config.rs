...
//! G-12-05). Gate: `EHRBASE_MULTIMEDIA_ENABLED` (default off).
//!
//! Loaded independently of the rest of the service (the sibling
//! `fhir`/`events`/`crate::signing` config pattern): a `figment` merge of
...
//! commit/read paths are byte-identical to today's inline behaviour (the
//! zero-drift gate) and no object store is ever contacted.
