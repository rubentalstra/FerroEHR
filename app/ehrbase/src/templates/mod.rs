...
//! - [`ingest`] — OPT 1.4 canonical XML → `OperationalTemplate` parse (S-05);
//!   the artefact is an `AUTHORED_RESOURCE`
//!   (`BASE/docs/resource/master02-resource_package.adoc` §Meta-data,
//!   S-01/S-02/S-03).
//! - [`store`] — the `template_store` repository (S-05/S-06/S-07/S-11):
...
//!   Templates), `moka`-cached (G-T05); the `WebTemplate` *format* is
//!   spec-silent and stays in `openehr-flat` (G-T06).
//!
