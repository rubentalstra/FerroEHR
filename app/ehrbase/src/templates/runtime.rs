...
//!   form is the [`WebTemplate`], memoised in a `moka` cache (G-T05).
//!
...
impl EhrbaseService {
    ...
    pub(crate) async fn web_template_for(
        ...
        // the commit-path 422 (G-T08). The XML load itself is not de-duplicated
    ...
    ///   template on a *commit* path (G-T08).
