...
//! config tree ([`crate::config::EhrbaseConfig`]); no loader of its own.
//!
...
pub struct StoreConfig {
    /// Persist every record locally (`EHRBASE__AUDIT__STORE__ENABLED`).
    /// **On by default** (owner posture: compliance out of the box).
    pub enabled: bool,
    /// Days to keep records; `0` = keep forever
    /// (`EHRBASE__AUDIT__STORE__RETENTION_DAYS`). Applied hourly by the
    /// retention reaper.
...
pub struct SyslogConfig {
    ...
    /// (`EHRBASE__AUDIT__SYSLOG__ENABLED`).
    pub enabled: bool,
    /// ARR host (`EHRBASE__AUDIT__SYSLOG__HOST`).
    pub host: String,
    /// ARR port (`EHRBASE__AUDIT__SYSLOG__PORT`).
    pub port: u16,
    /// Transport (`EHRBASE__AUDIT__SYSLOG__TRANSPORT`): `udp` | `tls`.
    pub transport: Transport,
    /// PEM file with the ARR CA to trust for TLS
    /// (`EHRBASE__AUDIT__SYSLOG__TLS_CA_FILE`).
    pub tls_ca_file: Option<String>,
    /// Client-certificate PEM file for mutual TLS
    /// (`EHRBASE__AUDIT__SYSLOG__TLS_IDENTITY_CERT_FILE`).
    pub tls_identity_cert_file: Option<String>,
    /// Client-key PEM file for mutual TLS
    /// (`EHRBASE__AUDIT__SYSLOG__TLS_IDENTITY_KEY_FILE`).
...
pub struct AuditConfig {
    /// Master switch (`EHRBASE__AUDIT__ENABLED`). **On by default** — every
