...
//!   `/management/env` snapshot, `ehrbase config check` output, or a debug log.
...
//! (`ehrbase::config`) into the `Secret` immediately after extraction, so
...
mod tests {
    ...
    fn secret_url_redacts_userinfo_but_keeps_connection_form() {
        let u = SecretUrl::new("postgres://ehrbase:ehrbase@localhost:5432/ehrbase");
        assert_eq!(
            u.expose(),
            "postgres://ehrbase:ehrbase@localhost:5432/ehrbase"
        );
        assert_eq!(u.redacted(), "postgres://***@localhost:5432/ehrbase");
        assert_eq!(format!("{u}"), "postgres://***@localhost:5432/ehrbase");
        assert!(!format!("{u:?}").contains("ehrbase:ehrbase"));
        assert_eq!(
            serde_json::to_string(&u).expect("serialize"),
            "\"postgres://***@localhost:5432/ehrbase\""
        );
    }
    ...
    fn secret_url_handles_amqp_vhost_and_no_userinfo() {
        ...
        let plain = SecretUrl::new("postgres://localhost:5432/ehrbase");
        assert_eq!(plain.redacted(), "postgres://localhost:5432/ehrbase");
