...
pub async fn execute(
    ...
    // variety, so caching it as named prepared statements both pollutes the
    // per-connection statement cache AND hands PostgreSQL a GENERIC plan
    // after five executions per connection — with `LIMIT`/`kind`/archetype
    // parameters unknown, the generic plan collapsed the streaming shape
    // (measured 5 ms mean → 19 s mean under sustained load, 2026-07-23).
    // An unnamed statement is custom-planned with the real values on every
    // execution (PostgreSQL docs: PREPARE / plan caching).
