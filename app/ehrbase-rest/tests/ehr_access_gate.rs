...
async fn app(name: &str, auth_enabled: bool, scheme: Option<Value>) -> Router {
...
async fn default_open_admits_anonymous() {
    let app = app("ehracc_default_open", false, None).await;
...
async fn restricted_gates_by_user_principal() {
    ...
    let app = app("ehracc_restricted_user", true, Some(scheme)).await;
...
async fn restricted_gates_by_role_principal() {
    ...
    let app = app("ehracc_restricted_role", true, Some(scheme)).await;
...
async fn privacy_ceiling_gates_composition_reads() {
    ...
    let app = app("ehracc_privacy", true, Some(scheme)).await;
...
async fn gate_keeper_guards_ehr_access_commits() {
    ...
    let app = app("ehracc_gatekeeper", true, Some(scheme)).await;
...
async fn gate_keeper_ignores_non_ehr_access_contributions() {
    ...
    let app = app("ehracc_gatekeeper_ignore", true, Some(scheme)).await;
