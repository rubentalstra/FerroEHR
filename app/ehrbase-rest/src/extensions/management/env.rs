...
mod tests {
    ...
    fn masks_dsn_credentials_keeps_host_and_db() {
        let cfg = json!({ "db": { "url": "postgres://ehrbase:hunter2@db.internal:5432/ehrbase?sslmode=require" } });
        let out = redact(&cfg);
        ...
        assert!(url.contains("/ehrbase"), "db name lost: {url}");
        assert_eq!(
            url,
            "postgres://***@db.internal:5432/ehrbase?sslmode=require"
        );
    }

    #[test]
    fn leaves_credential_free_urls_untouched() {
        let cfg = json!({ "issuer": "https://keycloak.example/auth/realms/ehrbase" });
        let out = redact(&cfg);
        assert_eq!(
            out["issuer"],
            "https://keycloak.example/auth/realms/ehrbase"
        );
    }
}
