...
//! directly on the spec. The crate's RBAC + ABAC ([`crate::access::authz`],
//! [`crate::dispatch::abac`]) are our **own enterprise extensions** — no
...
use crate::dispatch::RequestParts;
...
fn contribution_targets_ehr_access(parts: &RequestParts) -> bool {
    let Ok(value) = crate::negotiate::json_value(&parts.headers, &parts.body) else {
...
mod tests {
    ...
    fn open_ehr_admits_anonymous_and_named() {
        let s =
            settings(&json!({ "_type": "EHRBASE_ACCESS_CONTROL_V1", "default_access": "open" }));
