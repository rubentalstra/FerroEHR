...
//! configured role list (resolved in `ehrbase-rest`). [`authorize`] gates an
//! [`OperationClass`] against a principal's roles and the [`RbacConfig`].

use serde_json::{Map, Value};

use crate::access::authz::classify::OperationClass;
use crate::access::authz::config::{ManagementAccess, RbacConfig};
