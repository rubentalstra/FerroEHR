...
fn set_override_splits_on_the_first_equals_and_trims_the_key() -> Result<(), clap::Error> {
    let cli = Cli::try_parse_from(["ehrbase", "--set", "  db.url =postgres://h/db?a=b"])?;
    assert!(
...
fn set_override_rejects_a_pair_without_an_equals() {
    let err = Cli::try_parse_from(["ehrbase", "--set", "db.max_connections"])
...
/// `ehrbase` with no subcommand is the serve path (`command: None`).
#[test]
fn no_subcommand_selects_the_serve_path() -> Result<(), clap::Error> {
    let cli = Cli::try_parse_from(["ehrbase"])?;
    assert!(
...
fn config_path_is_global() -> Result<(), clap::Error> {
    const PATH: &str = "/etc/ehrbase/ehrbase.toml";
    for args in [
        ["ehrbase", "--config", PATH, "config", "check"],
        ["ehrbase", "config", "check", "--config", PATH],
    ] {
...
fn config_subcommands_parse() -> Result<(), clap::Error> {
    let default = Cli::try_parse_from(["ehrbase", "config", "default"])?;
    assert!(
        format!("{default:?}").contains("Default"),
        "not the Default utility: {default:?}"
    );
    let check = Cli::try_parse_from(["ehrbase", "config", "check"])?;
    assert!(
...
fn config_without_a_utility_is_rejected() {
    assert!(
        Cli::try_parse_from(["ehrbase", "config"]).is_err(),
...
fn healthcheck_url_is_optional_with_a_default() -> Result<(), clap::Error> {
    let explicit = Cli::try_parse_from(["ehrbase", "healthcheck", "--url", "http://h:8080/x"])?;
    assert!(
        format!("{explicit:?}").contains("http://h:8080/x"),
        "explicit URL lost: {explicit:?}"
    );
    let defaulted = Cli::try_parse_from(["ehrbase", "healthcheck"])?;
    assert!(
        format!("{defaulted:?}").contains("/ehrbase/rest/status"),
...
fn unknown_subcommand_is_rejected() {
    assert!(
        Cli::try_parse_from(["ehrbase", "migrate"]).is_err(),
        "an unknown subcommand must not parse"
    );
}

/// `ehrbase config default` runs end to end through the real dispatch: it only
...
async fn run_config_default_is_a_pure_stdout_path() -> anyhow::Result<()> {
    let cli = Cli::try_parse_from(["ehrbase", "config", "default"])?;
    run(cli).await
}
