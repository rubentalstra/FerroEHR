...
type Registry = Resource<Result<Option<Vec<TenantRow>>, ViewerError>>;
...
/// mutation report).
type CreateAction = Action<(String, String), (String, Result<TenantRow, ViewerError>)>;

/// The update action, reporting the same way.
type UpdateAction = Action<TenantEdit, (String, Result<TenantRow, ViewerError>)>;

/// The delete action, reporting the same way.
type DeleteAction = Action<TenantRow, (String, Result<(), ViewerError>)>;
...
pub fn TenantsPage() -> impl IntoView {
    ...
    let current: Resource<Result<Option<CurrentTenant>, ViewerError>> = Resource::new(
...
fn read_error(error: &ViewerError, object: &str) -> AnyView {
    match error {
        ViewerError::CdrUnauthorized(_) | ViewerError::Forbidden(_) => {
