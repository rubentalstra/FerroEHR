...
//! these journeys run against the composed stack, whose `docker/ehrbase.dev.toml`
...
//! management-ENABLED) and is unit-tested in `ehrbase_admin_ui::management`
...
const TEST_FILTER: &str = "ehrbase=debug,sqlx=warn";

/// The admin dev user (quickstart `docker/ehrbase.dev.toml`): the `loggers`
...
async fn admin_applies_and_resets_the_live_log_filter() {
    ...
    wait_text_contains(&h, "#ops-log-effective", "ehrbase=debug").await;
