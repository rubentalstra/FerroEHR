...
/// The admin dev user (quickstart `docker/ehrbase.dev.toml`) — the fixture
...
async fn delete_fixtures(cdr: &str) {
    ...
    for name in QUERY_NAMES {
        let status = http
            .delete(format!(
                "{cdr}/ehrbase/rest/openehr/v1/admin/query/{name}/{QUERY_VERSION}"
            ))
...
async fn seed_fixtures(cdr: &str, user: &str, pass: &str) {
    ...
    for name in QUERY_NAMES {
        let status = http
            .put(format!(
                "{cdr}/ehrbase/rest/openehr/v1/definition/query/{name}/{QUERY_VERSION}"
            ))
            .basic_auth(user, Some(pass))
