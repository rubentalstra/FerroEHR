...
//! pill polls the product status document (`/ehrbase/rest/status`: is the API
...
/// (`ehrbase::telemetry::prometheus`); a histogram is deliberately absent —
...
mod tests {
    ...
    fn build_info_lifts_the_spec_pins_into_their_own_section() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                ("name".to_owned(), "ehrbase".to_owned()),
    ...
    fn logger_view_reads_the_effective_and_boot_filters() {
        let view = logger_view(&serde_json::json!({
            "filter": "ehrbase=debug,sqlx=warn",
            "boot_filter": "info"
        }));
        assert_eq!(view.filter, "ehrbase=debug,sqlx=warn");
