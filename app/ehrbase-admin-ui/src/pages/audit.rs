...
#[cfg(feature = "ssr")]
fn parse_bundle(body: &str, offset: u32) -> Result<AuditPage, AdminUiError> {
    let bundle = serde_json::from_str::<serde_json::Value>(body)
        .map_err(|e| AdminUiError::Internal(format!("AuditEvent Bundle JSON: {e}")))?;
    let total = bundle
        .get("total")
    ...
    fn distils_the_balp_shape() {
        let resource = serde_json::json!({
            ...
            "agent": [
                ...
                { "requestor": false, "who": { "identifier": { "value": "ehrbase" } } },
                { "requestor": true, "policy": ["jti-1"] }
            ],
            "entity": [
                { "type": { "code": "1" }, "what": { "identifier": { "value": "patient-42" } } },
                { "type": { "code": "2" }, "what": { "identifier": { "value": "8fa1::ehrbase::1" } } }
            ]
        });
        ...
        assert_eq!(row.resource, "8fa1::ehrbase::1");
