...
//! (design §4.2). Boot validation is fail-closed: `pgp` mode without a loadable,
//! usable key refuses to construct.
...
/// spec's self-description requirement for the digest form).
...
impl Signer {
    ...
    pub fn from_config(config: &SigningConfig) -> Result<Self, SigningError> {
        let mode = match config.mode {
            ConfigMode::Digest => SignerMode::Digest,
            ConfigMode::Pgp => {
                let path = config
                    .key_path
                    .as_ref()
    ...
    /// Sign a Version's `canonical_form` (design §3.2): a SHA-256 digest
    /// (`sha256:<radix-64>`) or an armored RFC 4880 detached signature.
    ///
    /// # Errors
