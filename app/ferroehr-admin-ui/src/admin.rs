...
/// [`AdminUiError::Cdr`] / [`AdminUiError::Internal`] when the manifest cannot
/// be read.
#[server]
pub async fn probe_admin_api() -> Result<AdminAvailability, AdminUiError> {
...
mod tests {
    ...
    fn only_an_available_admin_api_renders_destructive_affordances() {
        ...
        assert!(!renders_admin_ops(&Err(AdminUiError::Unauthenticated)));
        assert!(!renders_admin_ops(&Err(AdminUiError::CdrUnreachable(
            "connection refused".to_owned()
        ))));
        assert!(!renders_admin_ops(&Err(AdminUiError::Internal(
    ...
    fn other_failures_name_the_object_and_their_own_next_action() {
        ...
        let expired = delete_failure_copy("EHR 7d44", &AdminUiError::Unauthenticated);
        assert!(expired.contains("sign in again"), "{expired}");

        let other = delete_failure_copy(
            "template `x`",
            &AdminUiError::Internal("serialization".to_owned()),
        );
        assert!(
