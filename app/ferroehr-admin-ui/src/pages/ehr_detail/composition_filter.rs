...
) -> Result<CompositionQuery, AdminUiError> {
...
///
/// # Errors
/// [`AdminUiError::Invalid`] naming `label` when the value is neither.
fn instant_bound(value: &str, day_suffix: &str, label: &str) -> Result<String, AdminUiError> {
    let refuse = || {
...
mod tests {
    ...
    fn an_unreadable_date_bound_is_refused_before_the_round_trip() {
        ...
        for bad in ["not-a-date", "2026-13-40", "yesterday", "2026-07-14T99:99"] {
            ...
            assert!(
                matches!(error, AdminUiError::Invalid(ref message) if message.contains("“from”")),
                "{error:?}"
            );
        }
        let error = composition_query(EHR, &CompositionFilter::new("", "", "nope", ""))
            .expect_err("an unreadable bound is refused");
        assert!(
            matches!(error, AdminUiError::Invalid(ref message) if message.contains("“to”")),
            "{error:?}"
        );
    }
}
