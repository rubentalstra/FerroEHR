...
fn version_preview(
    ...
    let tree = match serde_json::from_str::<Value>(&version.body) {
        ...
        Err(e) => inline_error(&AdminUiError::Internal(format!("version JSON: {e}"))),
    };
    let is_latest = version.is_latest;
    let label = format!("v{}", version.number);
...
pub(in crate::pages::ehr_detail::directory) fn time_travel_panel(
    at_time: Resource<Result<Option<DirectoryAtTime>, AdminUiError>>,
                    ...
                    match at_time.await {
                        ...
                        Ok(Some(DirectoryAtTime::Present(state))) => {
                            match serde_json::from_str::<Value>(&state.body) {
                                ...
                                Err(e) => {
                                    inline_error(
                                        &AdminUiError::Internal(format!("directory JSON: {e}")),
                                    )
                                }
                            }
                        }
...
pub(in crate::pages::ehr_detail::directory) fn path_panel(
    at_path: Resource<Result<Option<DirectorySubtree>, AdminUiError>>,
                    ...
                    match at_path.await {
                        ...
                        Ok(Some(DirectorySubtree::Found(body))) => {
                            match serde_json::from_str::<Value>(&body) {
                                ...
                                Err(e) => {
                                    inline_error(
                                        &AdminUiError::Internal(format!("subtree JSON: {e}")),
                                    )
                                }
                            }
                        }
