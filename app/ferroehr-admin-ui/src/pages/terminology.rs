...
//!
//! NOTE: no openEHR spec governs an admin UI — our own design / product
...
pub fn TerminologyPage() -> impl IntoView {
    ...
    let terminologies: Resource<Result<Option<Vec<String>>, AdminUiError>> =
        Resource::new(|| (), |()| async move { list_terminologies().await });
    let descriptor: Resource<Result<Option<TerminologyDescriptor>, AdminUiError>> = Resource::new(
        move || selected.get(),
        |id| async move {
            if id.is_empty() {
                Ok(None)
