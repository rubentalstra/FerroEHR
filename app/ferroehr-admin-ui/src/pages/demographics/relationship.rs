...
/// [`AdminUiError::Invalid`] on an empty id; CDR transport errors pass through;
...
pub async fn fetch_relationship(
    ...
    uid: String,
) -> Result<Option<RelationshipState>, AdminUiError> {
...
/// # Errors
/// [`AdminUiError::Unauthenticated`] without a console session;
/// [`AdminUiError::Invalid`] when a mandatory field is blank, an endpoint kind
...
/// # Errors
/// [`AdminUiError::Unauthenticated`] without a console session;
/// [`AdminUiError::Invalid`] on a missing version uid, a blank name, or a
...
/// # Errors
/// [`AdminUiError::Unauthenticated`] without a console session;
/// [`AdminUiError::Invalid`] when the id is not a full `OBJECT_VERSION_ID`; CDR
...
///
/// # Errors
/// [`AdminUiError::Invalid`] when `base` is not a JSON object, the name is
/// blank, the `details` draft is not an object, or the merged document cannot be
/// re-serialized.
fn apply_relationship_edits(base: &str, name: &str, details: &str) -> Result<String, AdminUiError> {
    let name = name.trim();
    if name.is_empty() {
        return Err(AdminUiError::Invalid(
    ...
    let details = parse_details_draft(details).map_err(AdminUiError::Invalid)?;
...
fn create_card(source_uid: &str, source_kind: &str) -> AnyView {
    ...
    let create: Action<RelationshipDraft, Result<String, AdminUiError>> =
...
type RelationshipResource = Resource<Result<Option<RelationshipState>, AdminUiError>>;
...
fn relationship_section(uid: Signal<String>, latest_version: RwSignal<String>) -> AnyView {
    let toaster = thaw::ToasterInjection::expect_context();
    let save: Action<RelationshipEdit, Result<String, AdminUiError>> =
...
fn edit_card(
    ...
    save: Action<RelationshipEdit, Result<String, AdminUiError>>,
) -> AnyView {
    let on_save = move |_| {
...
pub(super) fn party_relationships_section(
    kind: PartyKind,
    uid: Signal<String>,
    party: Resource<Result<Option<super::party::PartyState>, AdminUiError>>,
) -> AnyView {
    view! {
        <Transition fallback=table_skeleton>
            {move || Suspend::new(async move {
