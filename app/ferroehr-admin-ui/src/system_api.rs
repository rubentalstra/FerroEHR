...
/// [`AdminUiError::Cdr`] when the CDR answers non-2xx;
/// [`AdminUiError::Internal`] when the body is not the `Options` shape.
#[server]
pub async fn fetch_conformance_manifest() -> Result<ConformanceManifest, AdminUiError> {
    ...
    serde_json::from_str::<ConformanceManifest>(&body)
        .map_err(|e| AdminUiError::Internal(format!("conformance manifest JSON: {e}")))
}
