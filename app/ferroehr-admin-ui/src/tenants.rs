...
#[must_use]
pub fn renders_tenant_registry(probe: &Result<TenantAvailability, AdminUiError>) -> bool {
    probe
        .as_ref()
        .copied()
...
#[server]
pub async fn probe_tenant_registry() -> Result<TenantAvailability, AdminUiError> {
...
#[server]
pub async fn list_tenants() -> Result<Option<Vec<TenantRow>>, AdminUiError> {
    let session = crate::session::require_session().await?;
    let state: crate::state::AppState = expect_context();
    let url = state.cdr.rest_v1("admin/tenant");
    let response = state
    ...
    let value = serde_json::from_str::<serde_json::Value>(&body)
        .map_err(|e| AdminUiError::Internal(format!("tenant list JSON: {e}")))?;
    let rows = value
        .as_array()
...
/// when the body is not valid JSON.
#[server]
pub async fn fetch_current_tenant() -> Result<Option<CurrentTenant>, AdminUiError> {
    let session = crate::session::require_session().await?;
    let state: crate::state::AppState = expect_context();
    ...
    let body = crate::cdr::CdrClient::expect_success(response)?.body;
    let value = serde_json::from_str::<serde_json::Value>(&body)
        .map_err(|e| AdminUiError::Internal(format!("current tenant JSON: {e}")))?;
...
pub async fn update_tenant(
    ...
    if tenant_id.trim().is_empty() {
        return Err(AdminUiError::Invalid("no tenant id to update".to_owned()));
    }
    if !draft_is_complete(&name, &system_id) {
        return Err(AdminUiError::Invalid(
            "a tenant needs both a name and a system_id".to_owned(),
...
pub async fn delete_tenant(
    ...
    let session = crate::session::require_session().await?;
    if tenant_id.trim().is_empty() {
        return Err(AdminUiError::Invalid("no tenant id to delete".to_owned()));
    }
    let state: crate::state::AppState = expect_context();
    let url = state
        .cdr
...
#[cfg(feature = "ssr")]
fn stored_record(body: &str) -> Result<TenantRow, AdminUiError> {
    let value = serde_json::from_str::<serde_json::Value>(body)
        .map_err(|e| AdminUiError::Internal(format!("tenant record JSON: {e}")))?;
    Ok(tenant_row(&value))
}
...
mod tests {
    ...
    fn only_an_available_registry_renders_its_affordances() {
        ...
        assert!(!renders_tenant_registry(&Err(
    ...
    fn a_refusal_names_the_tenant_the_diagnostic_and_the_next_action() {
        ...
        let refused = tenant_failure_copy(
            "tenant `acme`",
            &AdminUiError::Forbidden("insufficient role".to_owned()),
        );
        assert!(
