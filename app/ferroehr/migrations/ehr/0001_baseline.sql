...
CREATE TABLE vo_version (
    ...
    -- Historical versions may carry none.
    signature       text,
    -- Whether `signature` was supplied verbatim by the committing client (an
    -- author signature over another agreed serialisation, or an IMPORTED_VERSION
    -- carrying its origin's signature — RM common master06 §Digital Signature /
    -- §Copying) rather than generated by this server. Client-supplied signatures
    -- are stored byte-for-byte and are NEVER re-verified at read (a foreign
    -- canonical form cannot be recomputed); read-time verification applies only
    -- to our own signatures. No openEHR spec governs read-time verification
    -- timing — our own design (server-side integrity hardening).
