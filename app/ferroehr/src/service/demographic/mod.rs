...
//!
//! Internal split — one file per concern, the domain files mirroring the SM
//! interface boundaries (SM `master06`):
//! - `types` — the public [`types::PartyKind`] resource-family key,
//! - `validate` — the inbound-body RM/BASE invariant checks (party +
//!   relationship) and the CONTRIBUTION-path validator entry points,
