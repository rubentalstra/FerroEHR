...
async fn service(db: &testkit::TestDb, config: &PrivacyConfig) -> FerroEhrService {
...
async fn an_opaque_subject_in_a_configured_namespace_is_stored_as_the_promoted_key() {
    ...
    let svc = service(&db, &enforcing()).await;
    let ehr_id = svc
...
async fn a_named_composer_is_accepted_by_default() {
    let db = testkit::db().await.expect("testkit database");
    let strict = service(&db, &PrivacyConfig::default()).await;
...
async fn strict_mode_refuses_a_composition_carrying_a_synthetic_bsn() {
    let db = testkit::db().await.expect("testkit database");
    let svc = service(&db, &PrivacyConfig::default()).await;
